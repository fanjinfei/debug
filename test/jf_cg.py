...
class MData():
    def __init__(self, data, outline = None):
        self.data = data
        print self.data[136]
        print self.dist(136, 69), self.dist(136, 101)
    ...
    def getDela(self): #initialization
        ...
        for [c,r] in tris:
                row.append(r)
                col.append(c)
                cd.append(self.dist(c,r))
        X = csr_matrix( (cd, (row, col)), shape=(len(self.data), len(self.data)))
        mx = minimum_spanning_tree(X).toarray().astype(float)
        mst = []
        ...
        return self.data, tris, hull, mst #edges
...
class Window(QtGui.QDialog):
    def __init__(self, parent=None):
        ...
        self.data, self.tris, self.hull, self.mst= self.md.getDela()
        self.opt = DFile(sys.argv[2]).data
