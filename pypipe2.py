...
def main_loop(master_fd, extras): #extra_input
    ...
    for extra_input in extras:
        ...
        while True:
            rfds, wfds, exceptionals = select.select(fds, [], [], 1)
            if not ( rfds or wfds or exceptionals): #timeout
                break
            if master_fd in rfds:
                data = os.read(master_fd, 1024)
                if not data:
...
def main():
    ...
    try:
        extras = extra_input.split(b'\r')
        extras = [ v+b'\r' for v in extras ]
        extras.append(b'quit\r')
        print(extras)
        main_loop(master_fd, extras) # extra_input
    except OSError:
        if restore:
