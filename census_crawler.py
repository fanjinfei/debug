...
class Crawler():
    ...
    def hangle_one_url(self, urls, data, failed_urls, error_urls, url, depth, c):
        md5 = hashlib.md5()
        if not c:
        ...
        while urls:
            ...
            for url,depth in urls.iteritems():
                if len(working_urls) < 20:
                    ...
                    working_url2.append(url)
                    self.handled_urls[url] = True
            for url in working_urls2:
                urls.pop(url)
            results = mlinks(working_urls) #read response
                ...
                if type(response) is Exception:
                    ...
                    for line in res.iter_lines():
                        ...
                        if line:
                            ...
                            rtext.append(decoded_line)
                    rtxt = u'\n'.join(rtext)
                handle_one_url(urls, data, failed_urls, error_urls, url, depth, rtxt)                
